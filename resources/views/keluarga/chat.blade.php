...
<div class="px-4 py-6 space-y-6">
    <div class="bg-white rounded-2xl shadow p-5 sm:p-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
                <h2 class="text-lg font-semibold text-gray-800">Pesan Petugas</h2>
                <p class="text-sm text-gray-500">
                    Percakapan terkait {{ $lansia->nama_lengkap }}
                </p>
            </div>
        </div>

        <div id="chat-messages"
             class="mt-4 space-y-3 max-h-[60vh] overflow-auto pr-1"
        ...
        <form id="chat-form"
              action="{{ route('keluarga.chat.store', $thread) }}"
              method="POST"
